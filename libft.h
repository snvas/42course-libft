...
/*   Updated: 2021/06/06 16:19:32 by snovaes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * De acordo com o manual, a função memset grava len bytes do valor c (convertidos para um caracter não assinado) na string b. 
 * Esta função retorna seu primeiro argumento. Começamos criando uma string char de p. Isso é o que usaremos para manter uma 
 * versão fundida de nosso parâmetro void b. Definimos p igual a uma versão char cast de b. Se você não está familiarizado 
 * com a conversão de tipo, é uma maneira de converter uma variável de um tipo de dados para outro tipo de dados. Em seguida, 
 * iniciamos nosso loop. ft_memset precisa confiar no tamanho de len que é passado para seu parâmetro e precisa trabalhar 
 * em uma parte geral da memória, não apenas em uma string terminada em '\ 0', então não podemos ter a condição de nosso loop 
 * com base na ideia usual de que chegaremos ao fim de uma string. Portanto, neste caso, dizemos que, desde que o len fornecido 
 * seja maior que 0, continuaremos o loop. Em seguida, retrocedemos pela string p, colocando o int c dentro de cada posição de índice. 
 * Fazemos len-1 porque estamos compensando o término '\ 0' no final da string que não queremos substituir. Nós decrementamos len e 
 * então começamos o loop novamente até que len não seja mais maior que 0. Então retornamos b.
 */
 ...
 * @param n -> n bytes to writing zeros Buffer size
 * De acordo com o manual, esta função grava n bytes zerados na string s. 
 * Se n for zero, ft_bzero não faz nada. Outra maneira de dizer isso é que faremos uma string de tamanho n 
 * e preencheremos cada posição de índice com um 0. Usamos nossa função ft_memset criada anteriormente para fazer isso. 
 * Revise ft_memset para entender como ele funciona e observe os parâmetros que fornecemos aqui quando o chamamos. 
 * Definimos com antecedência para preencher qualquer string de tamanho n com 0s.
 */
